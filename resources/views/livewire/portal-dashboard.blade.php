    ...
    <div class="app-container">
        ...
        <aside class="sidebar portal-sidebar" id="sidebar">
            ...
            <nav class="nav-group">
                ...
                <a href="#parent-dashboard" class="nav-item" data-view="parent-dashboard" id="nav-parent-dashboard" style="{{ $isParent ? '' : 'display:none;' }}">
            ...
            <div class="sidebar-footer"
                ...
                <button class="btn btn-outline"
                    ...
                    <i data-lucide="shield-check" style="width: 14px; vertical-align: middle;"></i> View Household
                </button>
                @endif
        ...
        <main class="main-content">
                ...
                <div style="display: grid; grid-template-columns: 2fr 1fr; gap: var(--space-8);">
                    ...
                    <div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-4);">
                            ...
                            <div class="card course-card-portal">
                                <span style="font-size: 11px; font-weight: 700; color: var(--color-primary-portal);">{{ $course['name'] }}</span>
                                <h4 style="margin: 4px 0 8px; font-family: 'Inter';">{{ $course['name'] }} — {{ $course['section'] }}</h4>
                                <div class="flex items-center gap-2" style="font-size: 12px; color: var(--color-body-gray);">
                                    <i data-lucide="clock" style="width: 14px;"></i> Next: {{ $course['next_class'] }}
                                </div>
                                <div class="progress-bar-portal">
                                    <div class="progress-fill-portal" style="width: {{ $course['progress'] }}%;"></div>
                                </div>
                                <div style="font-size: 11px; margin-top: 4px; text-align: right; color: var(--color-body-gray);">
                                    {{ $course['progress'] }}% Complete</div>
                            </div>
                            @empty
                            <div class="card" style="grid-column: 1 / -1; text-align: center; color: var(--color-body-gray);">
                        ...
                        <div style="display: grid; gap: var(--space-3);">
                            @forelse($feedItems as $item)
                                ...
                                <div class="card flex gap-4" style="border-left: 4px solid var(--color-deep-teal);">
                                    <div style="min-width: 40px; height: 40px; border-radius: 50%; background: #dcfce7; color: #166534; display: flex; align-items: center; justify-content: center;">
                                        <i data-lucide="play" style="width: 18px;"></i>
                                    </div>
                                    <div style="flex: 1;">
                                        <div class="flex justify-between">
                                            <h4 style="font-family: 'Inter'; font-size: 15px;">{{ $item['title'] }}</h4>
                                            <span style="font-size: 11px; color: var(--color-body-gray);">{{ $item['time'] }}</span>
                                        </div>
                                        <p style="font-size: 13px; color: var(--color-body-gray); margin-top: 4px;">{{ $item['description'] }}</p>
                                        <button class="btn btn-outline" style="font-size: 11px; padding: 4px 10px; margin-top: 8px;" onclick="window.location.hash = '{{ $item['action_route'] }}'">{{ $item['action_label'] }}</button>
                                    </div>
                                </div>
                                @elseif($item['type'] === 'grade')
                                <div class="card flex gap-4" style="border-left: 4px solid var(--color-mauve-rose);">
                                    <div style="min-width: 40px; height: 40px; border-radius: 50%; background: #fee2e2; color: #dc2626; display: flex; align-items: center; justify-content: center;">
                                ...
                                @endif
                            @empty
                                <div class="card" style="text-align: center; color: var(--color-body-gray);">No recent activity in your feed.</div>
                            @endforelse
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div style="display: grid; gap: var(--space-3);">
                            ...
                            <div class="flex items-center gap-3 p-3" style="background: white; border-radius: 8px; border: 1px solid var(--color-mid-gray);">
                                <div style="text-align: center; padding-right: 12px; border-right: 1px solid var(--color-mid-gray);">
                                    <div style="font-size: 14px; font-weight: 700;">{{ $deadline['date'] }}</div>
                                    <div style="font-size: 10px; text-transform: uppercase;">{{ $deadline['month'] }}</div>
                                </div>
                                <div>
                                    <div style="font-size: 13px; font-weight: 600;">{{ $deadline['title'] }}</div>
                                    <div style="font-size: 11px; color: {{ $deadline['color'] }};">{{ $deadline['due_in'] }}</div>
                                </div>
                            </div>
                            @empty
                            <div class="card" style="text-align: center; color: var(--color-body-gray);">No upcoming deadlines.</div>
                            @endforelse
                        </div>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <div class="card" style="margin-top: var(--space-6); background: var(--color-blush);">
                            ...
                            <p style="font-size: 12px; margin-bottom: 12px;">Complete the quiz to unlock the next
                                module.</p>
                            @if($isParent)
                            <button class="btn btn-primary" style="width: 100%; font-size: 12px; opacity: 0.5;" disabled>Locked (Parent View)</button>
                            @else
            ...
            <section id="view-quizzes" class="content-view hidden"
                x-data="{
                    timeRemaining: @entangle('quizTimeRemaining'),
                    active: @entangle('quizActive'),
                    timer: null,
                    get formattedTime() {
                ...
                x-init="$watch('active', val => { if (val) startTimer(); else if (timer) clearInterval(timer); })"
            >
                <div style="margin-bottom: var(--space-8); text-align: center;">
                    <h1 class="brand-font" style="font-size: 32px; color: var(--color-deep-navy);">Quiz 1: Pillars of Salah</h1>
                    <p style="color: var(--color-body-gray); font-variant-numeric: tabular-nums;">
                        <span x-show="active" style="font-weight:700;">Time Remaining: <span x-text="formattedTime"></span></span>
                        <span x-show="!active && !$wire.quizSubmitted">Start quiz to begin timer</span>
                        <span x-show="!active && $wire.quizSubmitted" style="color:#16a34a; font-weight:700;"><i data-lucide="check-circle" style="width:14px;vertical-align:middle;"></i> Quiz Submitted</span>
                    </p>
                </div>
                ...
                <div class="card" style="max-width: 600px; margin: 0 auto; position: relative;">
                    ...
                    <div style="display: grid; gap: var(--space-3);" :style="$wire.quizSubmitted ? 'opacity:0.6; pointer-events:none;' : ''">
                        <label style="display: flex; align-items: center; gap: var(--space-3); border: 1px solid var(--color-mid-gray); border-radius: 8px; cursor: pointer; padding: var(--space-3);" :style="$wire.quizAnswers.q1 === 'A' ? 'border: 2px solid var(--color-primary-portal); background: var(--color-blush);' : ''">
                            <input type="radio" value="A" wire:model.live="quizAnswers.q1">
                            <span>Wudu (Purification)</span>
                        </label>
                        <label style="display: flex; align-items: center; gap: var(--space-3); border: 1px solid var(--color-mid-gray); border-radius: 8px; cursor: pointer; padding: var(--space-3);" :style="$wire.quizAnswers.q1 === 'B' ? 'border: 2px solid var(--color-primary-portal); background: var(--color-blush);' : ''">
                            <input type="radio" value="B" wire:model.live="quizAnswers.q1">
                            <span>Ruku (Bowing)</span>
                        </label>
                        <label style="display: flex; align-items: center; gap: var(--space-3); border: 1px solid var(--color-mid-gray); border-radius: 8px; cursor: pointer; padding: var(--space-3);" :style="$wire.quizAnswers.q1 === 'C' ? 'border: 2px solid var(--color-primary-portal); background: var(--color-blush);' : ''">
                            <input type="radio" value="C" wire:model.live="quizAnswers.q1">
                    ...
                    <div class="card" data-status="pending" style="border-left: 4px solid #d97706;">
                        <div class="flex justify-between items-start">
                            ...
                            <div style="text-align: right; flex-shrink: 0; margin-left: var(--space-6);">
                                ...
                                <br>
                                @if($isParent)
                                <button class="btn btn-primary" style="font-size: 12px; background: var(--color-primary-portal); opacity: 0.5;" disabled>Locked (Parent View)</button>
                                @else
