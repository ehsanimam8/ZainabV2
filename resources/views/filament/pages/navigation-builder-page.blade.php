                ...
                <section id="view-cms-nav-builder" class="content-view">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-6);">
                        ...
                        <div class="card">
                            ...
                            <div id="nav-builder-list" wire:sortable="updateTaskOrder">
                                @foreach($navItems as $item)
                                <div class="nav-builder-item" wire:key="nav-{{ $item->id }}" style="display:flex;align-items:center;gap:12px;padding:12px 16px;border:1px solid var(--color-mid-gray);border-radius:8px;margin-bottom:8px;background:white;cursor:grab;">
                                    ...
                                    <button wire:click="removeItem('{{ $item->id }}')" style="background:none;border:none;cursor:pointer;color:#dc2626;font-size:16px;" title="Remove from nav">✕</button>
                                </div>
                                @endforeach
                            </div>
                        ...
                        <div class="card">
                            ...
                            <div style="display:flex;flex-direction:column;gap:8px;">
                                ...
                                <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border:1px dashed var(--color-mid-gray);border-radius:8px;">
                                    <div><span style="font-weight:600;font-size:14px;">{{ $page->title }}</span><span style="font-size:12px;color:var(--color-body-gray);margin-left:8px;">/{{ $page->slug }}</span></div>
                                    <button wire:click="addPage('{{ addslashes($page->title) }}', '{{ addslashes($page->slug) }}')" class="btn btn-outline" style="font-size:12px;padding:4px 12px;">+ Add</button>
                                </div>
                                @empty
                                <div style="font-size:13px;color:var(--color-body-gray);padding:10px;">No published pages available to link.</div>
                                @endforelse
                            </div>
                            <div style="margin-top:var(--space-6);padding-top:var(--space-4);border-top:1px solid var(--color-light-gray);">
                                ...
                                <div class="form-group" style="margin-bottom:8px;"><label>Label</label><input type="text" wire:model="customLabel" placeholder="e.g. Donate"></div>
                                <div class="form-group" style="margin-bottom:12px;"><label>URL</label><input type="text" wire:model="customUrl" placeholder="e.g. /donate or https://..."></div>
                                <button wire:click="addCustomLink" class="btn btn-primary" style="width:100%;">Add Custom Link</button>
                            </div>
                        </div>
                    </div>
                </section>
